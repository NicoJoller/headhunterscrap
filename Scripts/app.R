...
# Extrahierte und bereinigte Daten als 'results' definieren
# Beispielstruktur:
# results <- data.frame(
#   Firma = c("url1", "url2", "url3"),
#   Keyword = c("Mandat", "Erfolgsbasis", "Mandat"),
#   Präsenz = c("Found", "Not Found", "Found")
# )
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("selectedFirma", "Wählen Sie eine Firma:", choices = unique(results$Firma), selected = unique(results$Firma)[1]),
      selectInput("selectedKeyword", "Wählen Sie ein Keyword:", choices = unique(results$Keyword), selected = unique(results$Keyword)[1])
    ),
    mainPanel(
...
server <- function(input, output) {
  filteredData <- reactive({
    results %>%
      filter(Firma == input$selectedFirma, Keyword == input$selectedKeyword)
  })
  
  output$keywordPlot <- renderPlot({
    data <- filteredData()
    ggplot(data, aes(x = Keyword, fill = Präsenz)) +
      geom_bar(stat = "count") +
      scale_fill_manual(values = c("Found" = "green", "Not Found" = "red")) +  # Zuweisen der Farben "green" und "red"
      labs(title = paste("Präsenz des Keywords '", input$selectedKeyword, "' bei ", input$selectedFirma),
           x = NULL, y = "Anzahl")
  })
}
